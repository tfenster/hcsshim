...
func syscallWatcher(context logrus.Fields, syscallCompleted *bool) {
	...
	logrus.WithFields(context).
		...
		Warning("Syscall did not complete within operation timeout. This may indicate a platform issue. If it appears to be making no forward progress, obtain the stacks and see is there is a syscall stuck in the platform API for a significant length of time.")
}
